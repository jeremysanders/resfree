...
set pct_version "1.0.40 (2006-01-17)"

puts "Loading JSS common routines ${pct_version} (Jeremy Sanders 2003-2006)"
...
proc _pct_get_radii {} {
	...
	lappend retn "$radius $radius_err"
...
proc _pct_find_error { param delchi2 } {
    ...
    set retn [format "%e %e %e %s" $val [expr $maxerr-$val] [expr $minerr-$val] $flags]
...
proc _get_error_list_chan { chan delchi2 args } {
    set errors [_pct_get_error_list $args $delchi2]
...
proc pct_plot_phys_errors { channel normparm Tparm
	...
	puts $channel [format "%e %e %e %e %e" [expr {[lindex $rad 0]*$rs_kpc}]\
...
proc pct_plot_phys_channel { channel normparm Tparm Zparm
			     radiusscale_arcsec z outparam } {
    set no_datagrp [_pct_get_no_dg]
    set radii [_pct_get_radii]
...
proc pct_plot_density_channel { channel normparm Tparam Zparam
				radiusscale_arcsec z } {
...
proc pct_plot_tcool_channel { channel normparm Tparm Zparm
			      radiusscale_arcsec z} {
...
# calculate the cooling time from norm, T and Z params
proc pct_plot_entropy_channel { channel normparm Tparm Zparm
				radiusscale_arcsec z} {
    pct_plot_phys_channel $channel $normparm $Tparm $Zparm $radiusscale_arcsec \
	$z 3
    return
...
proc pct_plot_pressure_channel { channel normparam Tparam Zparam
				 radiusscale_arcsec z} {
...
proc pct_plot_gasmass_channel { channel normparam Tparam Zparam
				radiusscale_arcsec z} {
    pct_plot_phys_channel $channel $normparam $Tparam $Zparam $radiusscale_arcsec \
	$z 6
}
...
proc pct_plot_massrate_channel { channel normparam Tparam Zparam
				 radiusscale_arcsec z} {
    pct_plot_phys_channel $channel $normparam $Tparam $Zparam $radiusscale_arcsec \
	$z 7
}
...
proc fixnorm {} {
	...
	if { ${name} == "norm" } {
	    ...
	    set n_100 [expr {[lindex $xspec_tclout 0]*0.01}]
	    # trap out small parameters
	    if { $n_100 < 1e-7 } {
		set n_100 1e-7
	    }
	    set newparams [lreplace $xspec_tclout 1 1 $n_100]
	    newpar $p $newparams
	}
    }
}
...
exit & y"
    ...
    set throwaway [exec xspec $tempscriptname]
